...
const unsupportedModels = DEFAULT_MODEL_BASES.filter(
  (model) => !supportedModelIds.has(model.name),
);

if (process.env.NODE_ENV === "development" && unsupportedModels.length > 0) {
  console.info(
    `[WebLLM] Filtered ${unsupportedModels.length} unsupported model(s) from defaults.`,
  );
}

export const DEFAULT_MODELS: ModelRecord[] = DEFAULT_MODEL_BASES.filter((model) =>
  supportedModelIds.has(model.name),
).map((model) => ({
  ...model,
