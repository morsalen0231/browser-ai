...
export class WebLLMApi implements LLMApi {
  ...
  constructor(
    ...
    const engineConfig = {
      // cache type is injected later via setAppConfig once llmConfig is known
      appConfig: {
        ...prebuiltAppConfig,
    ...
    if (type === "serviceWorker") {
      ...
      if (typeof Worker === "undefined") {
        throw new Error(
          "Web Worker is unavailable in this runtime. Initialize WebLLMApi on the client only.",
        );
      }
      this.webllm = {
        type: "webWorker",
  ...
  private async initModel(onUpdate?: (message: string, chunk: string) => void) {
    ...
    // Apply cache preference (IndexedDB vs CacheStorage) now that llmConfig is available.
    this.webllm.engine.setAppConfig({
      ...prebuiltAppConfig,
